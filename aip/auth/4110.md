...
running in your GCP compute environment.  These identify either a service
...
### Retrieval and Caching of Workload Access Tokens from the Metadata Server
...
* If it is greater than 5 minutes, the token is fresh.  Proceed to use it.
* If it is less than 0, then the token is expired.  Refresh the cached token,
  then proceed with the current thread.
* If it is less than 5 minutes, but more than 0, then the token is stale.  You
  can handle the current thread in one of two ways:
...
Certain implementations of the metadata server (notably the one in use in GKE
when Workload Identity Federation for GKE is enabled) use the "Background
Refresh" strategy described above for managing their own internal caches of
tokens.  As long as your workload follows the robust refresh strategy described
above, this will not cause problems.
